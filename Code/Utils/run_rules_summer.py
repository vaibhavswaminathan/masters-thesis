...
def SUPTempSetpoint_rulelogic(X_vals, y_preds, y_truths):
    ...
    for idx in temp_idx:
        rounded_temps = list(np.around(np.array(X_vals[idx,:]),2))
...
def PumpOperating_rulelogic(X_vals, y_preds, y_truths, preds_rhvalve):
    """ Rule: RH ValveAct >= 20% implies RH PumpOperating = 1
        Seasonality: All
    """
    print("\n Running RULE 4")
    print(" Rule: RH ValveAct >= 20% implies RH PumpOperating = 1")
    ...
    # valve_idx = [idx for idx, x in enumerate(y_preds) if x == "Valve"]
    valve_idx = [idx for idx in preds_rhvalve['RH']]
    for o_idx in oper_idx:
        for v_idx in valve_idx:
            if y_truths[o_idx][1] == y_truths[v_idx][1]: # if the windows are same
                tstamps = [idx for idx, x in enumerate(X_vals[o_idx,:]) if x == 0.0] # find timestamps where pump operating = 1
                if tstamps: # check if list is not empty
                    matches = [True for x in X_vals[v_idx,tstamps] if x < 0.5] # check if valve > 20% 
                    overlap = len(matches) / len(X_vals[v_idx,tstamps])
                    if overlap > 0.8:
                        if v_idx not in valve_result_idx:
                            valve_result_idx.append(v_idx)
...
def PHPump_rulelogic(X_vals, y_preds, y_truths, rh_preds):
    """ Rule:  All Valves - PH Valves = RH Valves
        Seasonality: All
    """
    print("\n Running RULE 10")
    print("Rule : All Pump - RH Pump = PH Pump")
    ...
    y_truth_idx = [idx for idx, x in enumerate(y_truths[:,0]) if class_toapply in x]
    for i in oper_idx:
        if i not in rh_preds['RH'] and i not in CO_truth_idx:
            result_idx.append(i)
    rule_result = dict()
    rule_result[class_toapply] = result_idx
    return rule_result, y_truth_idx
...
if __name__ == "__main__":
    ...
    # Create lists for adding the sub-sys and component preds for each index in test-data set
    components = [None] * y_test.shape[0]
    subsystems = [None] * y_test.shape[0]
    ...
    for id in list(rule_preds_SUPTempSet.values())[0]:
        subsystems[id] = "SUP"
    # Confusion Matrix
    ...
    filter_ODATemp_preds['ODA'] = [x for x in rule_preds_ODATemp_6['ODA'] if x not in rule_preds_SUPTempSet['TempSUPSet']] # if a point has already been predicted as SUP Temp, do not consider it for ODA
    ...
    for id in list(filter_SUPTemp_preds.values())[0]:
        if subsystems[id] is None:
            subsystems[id] = "SUP"
    ...
    for id in list(filter_ODATemp_preds_1.values())[0]:
        if subsystems[id] is None:
            subsystems[id] = "ODA"
    # Confusion Matrix
    ...
    ## ---> RULE 4 ###
    rule_preds_RH_4, rule_preds_RHPump, trueidx_RH = PumpOperating_rulelogic(X_test, labellist, y_test, rule_preds_RH)
    # phpump_idx = [idx for idx, x in enumerate(y_test) if "PHPump" in x]
    rule4_RH = dict()
    rule4_RH['RH'] = list(set(rule_preds_RH_4['RH'] + rule_preds_RHPump['RH']))

    ph_acc = compute_rule_acc(rule_preds_RH_4, trueidx_RH)
    ph_preci = ph_acc['Precision']
    ph_rec = ph_acc['Recall']
    # print("PH true idxs: ", trueidx_PH)
    print("RH Valve --> Precision: {:.2f}, Recall: {:.2f}".format(ph_preci, ph_rec))
    ...
    # print("PH true idxs: ", trueidx_PH)
    ph_acc = compute_rule_acc(rule4_RH, trueidx_RH)
    ph_preci = ph_acc['Precision']
    ph_rec = ph_acc['Recall']
    print("RH (Pump + Valve) --> Precision: {:.2f}, Recall: {:.2f}".format(ph_preci, ph_rec))
    for id in list(rule4_RH.values())[0]:
        if subsystems[id] is None:
            subsystems[id] = "SUP"
            components[id] = "Heater"

    ### ---> RULE 10 ###
    rule_preds_PHPump, trueidx_PH = PHPump_rulelogic(X_test, labellist, y_test, rule_preds_RHPump)
    ...
    for id in list(rule_preds_PHPump.values())[0]:
        if subsystems[id] is None:
            subsystems[id] = "ODA"
            components[id] = "Heater"
    ...
    PH_preds = list(set(rule_preds_PHPump['PH'] + rule_preds_PH_5['PH']))
    ...
    # Speed values as Fan (component) and SUP (subsystem)
    speed_idx = [idx for idx, x in enumerate(labellist) if x == "Speed"]
    for id in speed_idx:
        if subsystems[id] is None:
            subsystems[id] = "SUP"
            components[id] = "Fan"

    none_subsys = sum(1 for item in subsystems if item is None)
    none_component = sum(1 for item in components if item is None)
    print(f"\nThere are {none_subsys} \"None\" sub-system values out of {len(subsystems)} total.")
    print(f"\nThere are {none_component} \"None\" component values out of {len(components)} total.")

    class_dict = dict()
    class_dict['substance'] = labellist
    class_dict['component'] = components
    class_dict['subsystem'] = subsystems

    # Open a file for writing in text mode (assuming UTF-8 encoding)
    preds_out_path = os.path.join(dataset+'_classes_dict.json')
