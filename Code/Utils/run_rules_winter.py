...
def PHValve_summer_rulelogic(X_vals, y_preds, y_truths, oda_preds):
    ...
    """
    print("\n Running RULE 5")
    print("Rule : ODATemp > 12°C implies PH ValveAct = 0%")
    applicable_class = ["Temperature", "Valve"]
    class_toapply = ["PH", "ODA"]
    valve_result_idx = []
    temp_result_idx = []
...
def delta_ODATemp_rulelogic(X_vals, y_preds, y_truths):
    """ Rule: If Max - Min > 7°C implies ODA Temp 
...
if __name__ == "__main__":
    ...
    # filter_SUPTempSet_preds = dict()
    # filter_SUPTempSet_preds['TempSUP'] = [x for x in rule_preds_SUPTempSet['TempSUPSet'] if x not in filter_SUPTemp_preds['TempSUP']]
